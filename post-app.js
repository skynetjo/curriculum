// ========================================
  ...
  const CURRENT_VERSION = '5.5.0'; // ✅ PERFORMANCE FIX + NOTIFICATION FIX
...
// ============================================================
// ✅ v5.5.2 OVERRIDE: Fixed AdminAttendanceAnalytics
// Appended to post-app.js (loads LAST — safely overrides app.js version)
// Fix: On-demand Firebase fetch for any date range
// ============================================================
...
function AdminAttendanceAnalytics({
  ...
  // ✅ v5.5.2 FIX: On-demand fetch for any date range
  const [extraData, setExtraData] = React.useState({ student: [], teacher: [] });
  const [loadingExtra, setLoadingExtra] = React.useState(false);
  const [loadedRange, setLoadedRange] = React.useState('');

  const loadDateRange = () => {
    const key = startDate + '|' + endDate;
    if (loadedRange === key) { alert('Data already loaded for this range!'); return; }
    ...
    Promise.all([fetchAll('studentAttendance'), fetchAll('teacherAttendance')])
      ...
      .catch(err => { console.error('[Dashboard]', err); alert('Failed to load: ' + err.message); })
  ...
  const filteredStudentAttendance = React.useMemo(() => {
  ...
  const filteredTeacherAttendance = React.useMemo(() => {
  ...
  const handleExportStudents = () => {
    ...
    if (filteredStudentAttendance.length === 0) { alert('No data found. Please click "Load Data" button first.'); return; }
  ...
  const handleExportTeachers = () => {
    if (loadingExtra) { alert('Still loading, please wait.'); return; }
    if (filteredTeacherAttendance.length === 0) { alert('No data found. Please click "Load Data" button first.'); return; }
  ...
  }, loadingExtra ? "\u23F3 Loading data from Firebase..." : (loadedRange ? "\u2705 Data Loaded — Click to Reload" : "\uD83D\uDD0D Load Data for Selected Date Range"))), React.createElement("div", {

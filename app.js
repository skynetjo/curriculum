// ✅ CURRICULUM TRACKER v5.2.0 - CHUNK 2: Admin + Attendance Features
...
function TeacherAttendanceDashboard({
  ...
  const handleExportStudents = async () => {
    // ✅ FIX v5.5.1: Fetch directly from Firebase for the selected date range.
    ...
    if (btn) { btn.innerText = '⏳ Fetching data...'; btn.disabled = true; }
    try {
      ...
      while (hasMore) {
        let query;
        if (mySchool && mySchool !== 'admin') {
          query = db.collection('studentAttendance')
      ...
      if (allRecords.length === 0) {
        alert(`No student attendance records found between ${startDate} and ${endDate}.`);
      ...
      const exportData = allRecords.map((a, i) => ({
        'S.No': i + 1,
        'Date': a.date,
        'Grade': a.grade || '-',
      ...
      console.log(`[Export] ✅ Exported ${allRecords.length} student attendance records`);
    } catch (err) {
      console.error('[Export] ❌ Error:', err);
      if (err.code === 'failed-precondition') {
        alert('Export needs a Firebase index. Please check the browser console for a link to create it automatically, then try again.');
      } else {
  ...
  const handleExportTeachers = async () => {
    // ✅ FIX v5.5.1: Fetch directly from Firebase for the selected date range.
    // Previously this only used in-memory data (last 30 days), so selecting
    // dates like Oct 2025 → Feb 2026 returned almost nothing.
    // Now we query Firebase fresh every time Export is clicked.
    ...
    // Show a loading message so teacher knows it's working
    const btn = document.activeElement;
    const originalText = btn ? btn.innerText : '';
    if (btn) { btn.innerText = '⏳ Fetching data...'; btn.disabled = true; }
    ...
    try {
      ...
      // Paginate: fetch 500 records at a time to handle large date ranges
      while (hasMore) {
        let query;
        if (mySchool && mySchool !== 'admin') {
          // Teacher/Manager: only their school
          query = db.collection('teacherAttendance')
            .where('school', '==', mySchool)
            .where('date', '>=', startDate)
            .where('date', '<=', endDate)   // ✅ inclusive end date
            .orderBy('date', 'asc')
            .limit(500);
        } else {
      ...
      if (allRecords.length === 0) {
        alert(`No teacher attendance records found between ${startDate} and ${endDate}.\n\nPlease check the dates and try again.`);
        if (btn) { btn.innerText = originalText; btn.disabled = false; }
        return;
      }

      // Sort by date then teacher name
      allRecords.sort((a, b) => {
        if (a.date < b.date) return -1;
        if (a.date > b.date) return 1;
      ...
      console.log(`[Export] ✅ Exported ${allRecords.length} teacher attendance records`);
